...
export default function FollowingPage() {
  ...
  // Fetch profile and following data
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch following using the username parameter
        const followingRes = await fetch(
          `/api/social/follow?username=${encodeURIComponent(username)}&type=following`
        );
        if (followingRes.ok) {
        ...
        if (session?.user?.email) {
          const currentFollowingRes = await fetch(
            `/api/social/follow?email=${encodeURIComponent(session.user.email)}&type=following`
          );
          if (currentFollowingRes.ok) {
            const currentFollowingData = await currentFollowingRes.json();
            const followingEmails = new Set(
              currentFollowingData.following?.map((f: FollowingUser) => f.email) || []
            );
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            <p className={`text-sm mb-4 ${isDarkTheme ? "text-slate-400" : "text-slate-500"}`}>
              {filteredFollowing.length} following{searchQuery && " found"}
            </p>
