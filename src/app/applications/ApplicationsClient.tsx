      ...
      <main className="flex-1 pt-24 pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              <h1 className="text-3xl font-bold text-[var(--foreground)]">My Applications</h1>
              <p className="text-[var(--muted)] mt-1">Track the status of your job applications</p>
            </div>
            <Link
              href="/dashboard"
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${
                  filter === tab.id
                    ...
                    : 'bg-[var(--card)] text-[var(--muted)] hover:text-[var(--foreground)] border border-[var(--border)] hover:border-[var(--border-hover)]'
                }`}
              >
                {tab.label}
                <span className={`px-2 py-0.5 rounded-full text-xs ${
                  filter === tab.id ? 'bg-white/20' : 'bg-[var(--background)]'
                }`}>
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {filteredApplications.map((app) => (
                <div
                  key={app.id}
                  className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-6 hover:border-[var(--border-hover)] transition-colors"
                >
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <Link
                          ...
                          className="text-lg font-semibold text-[var(--foreground)] hover:text-[#dc2626] transition-colors"
                        >
                          {app.job.title}
                        </Link>
          ...
          {applications.length > 0 && (
            <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[var(--foreground)]">{statusCounts.all}</p>
                <p className="text-sm text-[var(--muted)]">Total Applied</p>
              </div>
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-blue-600">{statusCounts.viewed}</p>
                <p className="text-sm text-[var(--muted)]">Viewed</p>
              </div>
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-emerald-600">{statusCounts.accepted}</p>
