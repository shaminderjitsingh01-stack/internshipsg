...
interface Props {
  userEmail: string;
  onShare?: (streakCount: number) => void;
}
    ...
    <div className="bg-gradient-to-br from-orange-500 to-red-600 rounded-xl sm:rounded-2xl p-4 sm:p-6 text-white relative overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="flex items-end gap-2 mb-3 sm:mb-4">
          <span className="text-4xl sm:text-5xl font-bold">{currentStreak}</span>
          <span className="text-lg sm:text-xl mb-1 sm:mb-2 opacity-80">day{currentStreak !== 1 ? "s" : ""}</span>
        </div>
        ...
        {nextBadge && (
          <div className="mb-3 sm:mb-4">
            <div className="flex items-center justify-between text-xs sm:text-sm mb-1">
              ...
              <span className="opacity-80">{nextBadge.daysRemaining} more day{nextBadge.daysRemaining !== 1 ? "s" : ""}</span>
            </div>
          ...
          <div className="pt-3 sm:pt-4 border-t border-white/20">
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              {(showAllBadges ? badges : badges.slice(0, 3)).map((badge) => (
                <div
                  key={badge.id}
                  className="flex items-center gap-1 bg-white/20 px-2 py-1 rounded-full text-xs sm:text-sm"
        ...
        {currentStreak >= 1 && onShare && (
          <button
            ...
            className="mt-3 sm:mt-4 w-full py-2 bg-white text-red-600 rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm hover:bg-white/90 transition-colors flex items-center justify-center gap-2"
          >
